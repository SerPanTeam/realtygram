...
export function ChatList({ selectedUserId, onSelectUser }: ChatListProps) {
  ...
  const loadChatsFromServer = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.panchenko.work/chat/conversations",
        {
          method: "GET",
          headers: {
          ...
          try {
            ...
            const otherUserId =
              ...
              (conversation.participants &&
                conversation.participants.find((id: number) => id !== user.id));

            if (!otherUserId) {
              console.error(
                "Could not determine other user ID for conversation:",
                conversation
              );
              continue;
            }
            ...
            const roomId =
              conversation.roomId ||
              `room_${Math.min(user.id, otherUserId)}_${Math.max(
                user.id,
                otherUserId
              )}`;
      ...
      setError(
        "Не удалось загрузить список чатов. Пожалуйста, попробуйте позже."
      );
  ...
  const loadChatsAlternative = async () => {
    ...
    try {
      ...
      const messagesByUser = new Map<
        number,
        { messages: ChatMessage[]; profile?: any }
      >();
      ...
      if (messages.length > 0) {
        for (const message of messages) {
          ...
          if (!senderId || !recipientId) {
            console.warn(
              "Skipping message without sender or recipient:",
              message
            );
            continue;
          }
          ...
          // Определяем ID собеседника (не текущего пользователя)
          const otherUserId =
            Number(senderId) === user.id
              ? Number(recipientId)
              : Number(senderId);
          const otherProfile =
            Number(senderId) === user.id ? recipientProfile : senderProfile;
      ...
      for (const [otherUserId, userData] of messagesByUser.entries()) {
        try {
          ...
          const unreadCount = sortedMessages.filter(
            (msg) => !msg.read && Number(msg.senderId) !== user.id
          ).length;
  ...
  const addSampleChat = () => {
    ...
    const sampleProfile: Profile = {
      ...
      bio: "Welcome to RealtyGRAM! Send me a message to start chatting.",
    ...
    setRecentChats([
      {
        ...
        lastMessage: {
          id: 1,
          content: "Welcome to RealtyGRAM! Click here to start a conversation.",
          senderId: sampleProfile.id,
          recipientId: user.id,
          roomId: `room_${Math.min(user.id, sampleProfile.id)}_${Math.max(
            user.id,
            sampleProfile.id
          )}`,
      ...
      try {
        ...
        setRecentChats((prevChats) => {
          ...
          const existingChatIndex = prevChats.findIndex(
            (chat) => chat.profile.id === profile.id
          );
    ...
    const searchUsers = async () => {
      ...
      try {
        ...
        try {
          ...
          const response = await fetch(
            `https://api.panchenko.work/users/search?username=${encodeURIComponent(
              searchQuery
            )}`,
            {
              headers: {
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
              ...
              {searchResults.map((profile) => (
                ...
                >
                  <Avatar className="h-12 w-12 mr-3">
                    <AvatarImage
                      src={formatImageUrl(profile.img)}
          ...
          <div>
            {recentChats.map((chatItem) => (
              <div
                ...
                className={`flex items-center p-4 cursor-pointer hover:bg-gray-50 ${
                  selectedUserId === chatItem.profile.username
                    ? "bg-gray-100"
                    : ""
                }`}
                onClick={() => {
              ...
              >
                <Avatar className="h-12 w-12 mr-3 relative">
                  <AvatarImage
                    src={formatImageUrl(chatItem.profile.img)}
                    alt={chatItem.profile.username}
                  />
                  <AvatarFallback>
                    {chatItem.profile.username.slice(0, 2).toUpperCase()}
                  </AvatarFallback>
          ...
          <div className="p-4 text-center text-[#737373]">
            ...
            <Button
              variant="link"
              className="text-[#0095f6] p-0 h-auto mt-1"
